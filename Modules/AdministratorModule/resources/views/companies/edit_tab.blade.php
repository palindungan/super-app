<div class="text-center">
    <div class="card d-inline-block">
        <div class="card-body">
            <ul class="nav nav-pills nav-secondary nav-pills-no-bd nav-pills-icons">
                <li class="nav-item">
                    <a data-bs-toggle="pill" class="nav-link active" id="nav-index-0-tab" href="#nav-index-0">
                        Cabang
                    </a>
                </li>
                <li class="nav-item">
                    <a data-bs-toggle="pill" class="nav-link" id="nav-index-1-tab" href="#nav-index-1">
                        Pengguna
                    </a>
                </li>
                <li class="nav-item">
                    <a data-bs-toggle="pill" class="nav-link" id="nav-index-2-tab" href="#nav-index-2">
                        Saldo
                    </a>
                </li>
            </ul>
        </div>
    </div>
</div>

<div class="tab-content mt-3 mb-3" id="nav-tabContent">
    <div role="tabpanel" class="tab-pane fade show active" id="nav-index-0" aria-labelledby="nav-index-0-tab">
        Cabang
    </div>
    <div role="tabpanel" class="tab-pane fade " id="nav-index-1" aria-labelledby="nav-index-1-tab">
        Pengguna
    </div>
    <div role="tabpanel" class="tab-pane fade " id="nav-index-2" aria-labelledby="nav-index-2-tab">
        Saldo
    </div>
</div>
